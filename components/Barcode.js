...
function BarcodeChunk(props) {
    ...
    return bars.map((bar, i) => {
        return <Rect key={i} x={bar.x} y={bar.y} width={bar.width} height={bar.height} />;
    });
}
...
function Barcode({value, options, rotation}) {
    ...
    return (
        <Svg
            ...
            rotation={rotation}
        >
            {options.background && <Background width={width} height={maxHeight} color={options.background} />}
